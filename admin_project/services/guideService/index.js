const guideController = require("../../controller/guideController/index.js");
...
const guideService = {
  ...
  uploadPdf: async (req, res) => {
    ...
    try {
      const pdfPath = `http://localhost:3000/uploads/guidePDF/${req.file.filename}`;
      const result = await guideController.uploadPdf(pdfPath, data);
  ...
  getAllApply: async (req, res) => {
    const result = await guideController.getAllApply();
    res.json(returnResult.success(result));
  },
  ...
  pass: async (req, res) => {
    ...
    const result = await guideController.pass(data);
    res.json(returnResult.success(result));
  },
  ...
  reject: async (req, res) => {
    ...
    const result = await guideController.reject(data);
    res.json(returnResult.success(result));
  },
  ...
  completeInfo: async (req, res) => {
    ...
    const guideAvatar = file
      ? `http://localhost:3000/uploads/guide_avatars/${file.filename}`
    ...
    const result = await guideController.completeInfo(id, data);
    res.json(returnResult.success(result));
  },
  ...
  // 根据导游id获取导游信息
  getGuideInfoById: async (req, res) => {
    const { id } = req.query;
    const result = await guideController.getGuideInfoById(id);
    res.json(returnResult.success(result));
  },

  // 获取所有导游信息
  getAllGuide: async (req, res) => {
    const { pageSize, currentPage } = req.query;
    const skipNum = (currentPage - 1) * pageSize;
    const result = await guideController.getAllGuide(pageSize, skipNum);
    res.json(returnResult.success(result));
  },
