...
const guideService = require("../../services/guideService/index.js");
...
const pdfStorage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, path.join(__dirname, "../../public/uploads/guidePDF"));
  },
...
const avatarStorage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, path.join(__dirname, "../../public/uploads/guide_avatars"));
  },
  filename: function (req, file, cb) {
    cb(null, Date.now() + "_" + file.originalname);
  },
...
// 上传证书
router.post("/uploadPdf", uploadPdf.single("pdf"), guideService.uploadPdf);

// 获取所有的申请信息
router.get("/allApply", guideService.getAllApply);

//根据id获取申请信息
router.get("/applyById", guideService.getApplyById);

// 通过导游申请
router.put("/pass", guideService.pass);

// 拒绝导游申请
router.put("/reject", guideService.reject);

// 完善导游信息
router.put(
  "/completeInfo",
  uploadAvatar.single("newAvatar"),
  guideService.completeInfo
);

// 获取所有导游信息
router.get("/allGuide", guideService.getAllGuide);
// 根据导游id新增产品
router.post(
  "/addProduct",
...
// 根据id获取导游信息
router.get("/getGuideInfoById", guideService.getGuideInfoById);
