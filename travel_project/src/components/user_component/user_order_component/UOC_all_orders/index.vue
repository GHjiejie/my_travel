<template>
  <div class="allOrders">
    <template v-if="effectiveOrders.length !== 0">
      <div class="orderList">
        <div class="orderItem" v-for="(item, index) in allOrders.orderList">
...
// 获取当前用户的所有订单
onBeforeMount(async () => {
  if (loginUserInfoStore.repay_order_id != '') {
    // 重新查询订单状态
    await requeryOrderState(loginUserInfoStore.repay_order_id);
  }
  userId.value = loginUserInfoStore.login_user_info._id;
  orderId.value = loginUserInfoStore.lastTime_pay_order_id;
  if (orderId.value == '' || orderId.value == loginUserInfoStore.thisTime_pay_order_id) {
    // console.log('订单号相同，不需要重新获取');
    if (loginUserInfoStore.user_orders) {
      // console.log('用户订单信息已存在store里面');
      allOrders.value = loginUserInfoStore.user_orders;
      effectiveOrders.value = loginUserInfoStore.user_effective_orders;

    } else {
      console.log('用户订单信息不存在store里面');
      // 获取当前用户的订单信息
      await getAllOrders();
    }
    ...
    console.log('订单号不同，需要重新获取');
    loginUserInfoStore.lastTime_pay_order_id = loginUserInfoStore.thisTime_pay_order_id;

    await updateOrder(loginUserInfoStore.lastTime_pay_order_id);
  }
});
// 查看更多
...
const updateOrder = async (order_id) => {
  ...
  if (res.data.code == 200) {
    loginUserInfoStore.user_orders = res.data.data;
    orderId.value = loginUserInfoStore.lastTime_pay_order_id;
    // allOrders.value.orderList = allOrders.value.orderList.filter(item => item.code == 10000 || item.code == 10001);
    // loginUserInfoStore.user_orders = allOrders.value;
  }
}
watch(() => loginUserInfoStore.user_orders, (newVal, oldVal) => {
  allOrders.value = newVal;
  loginUserInfoStore.user_effective_orders = newVal.orderList.filter(item => item.code == 10000 || item.code == 10001);
  // allOrders.value.orderList = newVal.orderList.filter(item => item.code == 10000 || item.code == 10001);
  effectiveOrders.value = loginUserInfoStore.user_effective_orders;
  console.log('有效订单', effectiveOrders.value);
  // console.log('new订单', newVal);
  // console.log('old订单', oldVal);
}, {
  deep: true
});
