    ...
    <div class="right">
      <div class="chatBox" ref='chatBoxRef'>
        ...
        <div v-for="(message, index) in messages" :key="index"
          ...
          <!-- <div class="msgContent">{{ message.text }}</div> -->
          <div class="msgContent" v-html="message.text"></div>
        </div>
      ...
      <div class="toolBox" ref="toolBoxRef">
        ...
        <div class="inputBox" @keyup.enter="sendMessage">
          <!-- 处理文本 -->
          <input type="text" class="inputContent" v-model="msg" contenteditable="true">
          <!-- 处理图片 -->
          <div class="sendPhoto">
            <img :src="item" alt="" v-for="item in photoList">
          </div>
        </div>
...
<script setup>
  import { ref, onBeforeMount, nextTick, onMounted, unref } from 'vue';
  ...
  const handlePhoto = async (files) => {
    ...
    console.log(photoList.value.path_collection)
    ...
    // messages.value.push({ text: res.data.data, type: "sent", avatar: loginUserInfo.login_user_info.avatar, username: loginUserInfo.login_user_info.username, messageType: messageType.value });
    // nextTick(() => {
    //   scrollToBottom();
    // });
  }
  ...
  const getHistoryMessages = async () => {
    ...
    for (let i = 0; i < historyMessages.value.length; i++) {
      if (historyMessages.value[i].sender === loginUserInfo.login_user_info._id) {
        messages.value.push({ text: historyMessages.value[i].message, type: "sent", avatar: historyMessages.value[i].avatar, username: historyMessages.value[i].username });
      } else {
        messages.value.push({ text: historyMessages.value[i].message, type: "received", avatar: historyMessages.value[i].avatar, username: historyMessages.value[i].username });
      }
    }
    nextTick(() => {
      scrollToBottom();
  ...
  const sendMessage = () => {
    ...
    if (msg.value.trim() !== '') {
      ...
      messages.value.push({ text: msg.value, type: "sent", avatar: loginUserInfo.login_user_info.avatar, username: loginUserInfo.login_user_info.username, messageType: messageType.value });
      msg.value = '';
  ...
  socket.on('privateMessage', (message) => {
    ...
    if (message.sender === user_id || message.receiver === loginUserInfo.login_user_info._id) {
      messages.value.push({ text: message.message, type: "received", avatar: message.avatar, username: message.username, messageType: message.messageType });
      nextTick(() => {
        scrollToBottom();
      });
    }
  ...
  // const receivedMsg = () => {
  //   messages.value.push({ text: "Hello Jack~~", type: "received" });
